...
<img src="/v190/docs/assets/o-mysql.png" alt="OpenEBS and MySQL" style="width:400px;">
...
<img src="/v190/docs/assets/svg/mysql-deployment.svg" alt="OpenEBS and Percona" style="width:100%;">
...
As in most cases, cStor pool may not be dedicated to just MySQL alone. It is recommended to watch the pool capacity and add more disks to the pool before it hits 80% threshold. See [cStorPool metrics](/v190/docs/next/ugcstor.html#monitor-pool) 

**Maintain volume replica quorum during node upgrades**

 cStor volume replicas need to be in quorum when MySQL application is deployed as `deployment` and cStor volume is configured to have `3 replicas`. Node reboots may be common during Kubernetes upgrade. Maintain volume replica quorum in such instances. See [here](/v190/docs/next/k8supgrades.html) for more details.
