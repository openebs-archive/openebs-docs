...
OpenEBS has cureently two storage Engine,Jiva and cStor which can used to provision volume. Jiva requires the disk to be mounted (i.e., attached, formatted with a fs & mounted) and cStor can consume disks that are attached (are visible to OS as SCSI devices) by creating a pool with these disks.
