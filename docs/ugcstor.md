...
**Note**: Post provisioning OpenEBS cStor volume, resiliency of the application can be checked by injecting chaos. Litmus is a Cloud-native Chaos Engineering tool which can be used to validate various chaos tests on Kubernetes in a cloud-native way. Some of the Litmus based chaos tests are mentioned [here](/docs/next/chaostest.html).
