...
If you are installing OpenEBS in CentOS or OpenShift,you must need to grant privileges to ndm pods. For installing OpenEBS in OpenShift environment,more details can be read [here](/v170/docs/next/kb.html#OpenEBS-install-openshift-4.1).
...
For LocalPV based on device, details of disk mount status can be obtained [here](/v170/docs/next/uglocalpv.html#General-verification-for-disk-mount-status-for-Local-PV-based-on-device).
...
By using manual method, you must give the selected disk name which is listed by NDM. This details has to be entered in the StoragePoolClaim YAML under `diskList`. See [storage pool](/v170/docs/next/ugcstor.html#creating-cStor-storage-pools) for more info. 
...
By auto method, its not need to provide the disk details in the StoragePoolClaim YAML. You have to specify `maxPools` count to limit the storage pool creation in OpenEBS cluster and `poolType` for the type of storage pool such as Mirrored or Striped.  See [storage pool](/v170/docs/next/ugcstor.html#creating-cStor-storage-pools) for more info.
...
### [Uninstall](/v170/docs/next/uninstall.html)
