...
For LocalPV based on device, details of disk mount status can be obtained [here](/v200/docs/next/ndm.html).
...
By using manual method, you must give the selected disk name which is listed by NDM. This details has to be entered in the StoragePoolClaim YAML under `diskList`. See [storage pool](/v200/docs/next/ugcstor.html#creating-cStor-storage-pools) for more info. 
...
By auto method, its not need to provide the disk details in the StoragePoolClaim YAML. You have to specify `maxPools` count to limit the storage pool creation in OpenEBS cluster and `poolType` for the type of storage pool such as Mirrored or Striped.  See [storage pool](/v200/docs/next/ugcstor.html#creating-cStor-storage-pools) for more info.
...
### [Uninstall](/v200/docs/next/uninstall.html)
