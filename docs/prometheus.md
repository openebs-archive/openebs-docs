...
<img src="/v250/docs/assets/o-prometheus.png" alt="OpenEBS and Prometheus" style="width:400px;">
...
<img src="/v250/docs/assets/svg/prometheus-deployment.svg" style="width:100%;">
...
1. **Install OpenEBS :** If OpenEBS is not installed on the Kubernetes already, start by <a href="/v250/docs/next/installation.html" target="_blank">installing</a> OpenEBS on all or some of the cluster nodes. If OpenEBS is already installed, go to step 2.
...
As in most cases, cStor pool may not be dedicated to just Prometheus alone. It is recommended to watch the pool capacity and add more disks to the pool before it hits 80% threshold. See [cStorPool metrics](/v250/docs/next/ugcstor.html#monitor-pool) 



**Maintain volume replica quorum during node upgrades**

 cStor volume replicas need to be in quorum Prometheus application is deployed as `deployment` and cStor volume is configured to have `3 replicas`. Node reboots may be common during Kubernetes upgrade. Maintain volume replica quorum in such instances. See [here](/v250/docs/next/k8supgrades.html) for more details.
