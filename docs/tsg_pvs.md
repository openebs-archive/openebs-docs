...
These steps are required where replicas could get scheduled onto a node where the data doesn't exist.
...
Deploy a Recovery PVC with a **single replica**.
...
Replace **replica hostname** in patch-replica-dep-nodename.json with the _replica-hostname_ that was obtained in **Step 1**. It is the node where source/backuped up replica data is available. If the backup data is available on a remote machine, you can set the hostname to the current node where Replica is running. 
