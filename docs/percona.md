...
2. **Configure cStor Pool**
   ...
   If cStor Pool is not configured in your OpenEBS cluster, this can be done from [here](/v160/docs/next/ugcstor.html#creating-cStor-storage-pools). If cStor pool is already configured, go to the next step. Sample YAML named **openebs-config.yaml** for configuring cStor Pool is provided in the Configuration details below.
...
As in most cases, cStor pool may not be dedicated to just Percona database alone. It is recommended to watch the pool capacity and add more disks to the pool before it hits 80% threshold. See [cStorPool metrics](/v160/docs/next/ugcstor.html#monitor-pool). 
