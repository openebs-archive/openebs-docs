...
![OpenShift in OperatorHub](/v230/docs/assets/openshift-operatorhub.png)

This guide recommends installing the operator into an empty `openebs`
namespace.

![OpenShift in OperatorHub](/v230/docs/assets/openshift-operator-installnamespace.png)

#### Installation option: via "manual" install</h4>

1. Find the latest OpenEBS release version from [here](/v230/docs/next/releases.html) and download the latest OpenEBS operator YAML in your master node. The latest openebs-operator YAML file can be downloaded using the following way.
...
- **Installing OpenEBS**
  ...
  - The default StorageClass `openebs-hostpath` can be used to create local PV on the path `/var/openebs/local` in your Kubernetes node. You can either use `openebs-hostpath` storage class to create volumes or create new storage class by following the steps mentioned [here](/v230/docs/next/uglocalpv-hostpath.html).
