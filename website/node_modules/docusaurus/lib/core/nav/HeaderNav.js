...
class HeaderNav extends React.Component {
  ...
  fetchVersions() {
    var versionsObj = {};
    const version = JSON.parse(fs.readFileSync(CWD + '/versions.json', 'utf8'));
    var versions = version.versions;
    Object.keys(versions).sort(function(a,b) {return b-a;}).forEach(vr => {
      versionsObj[vr] = versions[vr];
    })
    return versionsObj;
  }
          ...
          <header>
            <a
              ...
              {siteConfig.headerIcon && (
                <img
                  className="logo-text"
                  src={this.props.baseUrl + siteConfig.headerText}
                  alt={siteConfig.title}
                />
              )}
            ...
            {env.versioning.enabled && (
                <div className="dropdown">
                  <span className="ver-link">{Object.keys(this.fetchVersions())[0]}</span>
                  <span className="ver-link-span">
                  <i className="arrow down"></i>
                  </span>
                  <div className="dropdown-menu" id="versions-list" aria-labelledby="dropdownMenuButton">
                    {Object.keys(this.fetchVersions()).map((version, i) => {
                      return <a className="dropdown-item" id={`${i}`} href={this.fetchVersions()[`${version}`][1]}>{version}</a>;
                    })}
                  </div>
               </div>
            )}
